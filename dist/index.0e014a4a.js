...
})({"e6DQT":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
_storageJs.loadStorage();
_uiJs.UI.ALWAYS_ON_DISPLAY.searchBtn.addEventListener("click", (e)=>{
    e.preventDefault();
    _weatherRenderJs.sendForecastRequest(_uiJs.UI.ALWAYS_ON_DISPLAY.searchedCity.value);
    _weatherRenderJs.sendWeatherRequest(_uiJs.UI.ALWAYS_ON_DISPLAY.searchedCity.value);
});
...
},{}],"22C9S":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "favoriteCities", ()=>favoriteCities
);
parcelHelpers.export(exports, "sendWeatherRequest", ()=>sendWeatherRequest
);
parcelHelpers.export(exports, "sendForecastRequest", ()=>sendForecastRequest
);
parcelHelpers.export(exports, "addCity", ()=>addCity
);
parcelHelpers.export(exports, "renderStoredFavorites", ()=>renderStoredFavorites
);
var _apiURLJs = require("./apiURL.js");
var _storageJs = require("./storage.js");
var _converterJs = require("./converter.js");
var _uiJs = require("./UI.js");
let favoriteCities = [];
async function sendWeatherRequest(city) {
    try {
        const weatherUrl = _apiURLJs.getUrl(city, "weather");
        const response = await fetch(weatherUrl);
        const responseData = await response.json();
        const weatherData = new WeatherData(responseData);
        renderCurrentWeather(weatherData);
        renderWeatherDetails(weatherData);
        _storageJs.storage.saveCurrentCity(city);
        _uiJs.UI.ALWAYS_ON_DISPLAY.searchedCity.value = "";
    } catch (err) {
        alert(err);
    }
}
async function sendForecastRequest(city) {
    try {
        const forecastUrl = _apiURLJs.getUrl(city, "forecast");
...
function WeatherData(data) {
    ...
    this.sunRise = _converterJs.convertTime(data.sys.sunrise);
    this.sunSet = _converterJs.convertTime(data.sys.sunset);
}
function getIcon(id) {
    _uiJs.UI.CURRENT.imgweathericon.src = `https://openweathermap.org/img/wn/${id}@4x.png`;
}
function renderWeatherDetails(weatherData) {
    for(let key in _uiJs.UI.DETAILS)_uiJs.UI.DETAILS[key].textContent = weatherData[key];
}
...
function renderForecast(forecastDaysArray) {
    _uiJs.UI.FORECAST.forecastScrollBox.innerHTML = "";
    forecastDaysArray.slice(0, 11).forEach((item)=>{
        const forecastBox = document.createElement("div");
        forecastBox.innerHTML = `
      <div class="date">${_converterJs.convertDate(item.dt)}</div>
      <div class="forecasttemp">Temperature: ${Math.round(item.main.temp)}°</div>
      <div class="forecastfeelslike">Feels like: ${Math.round(item.main.feels_like)}°</div> 
      <div class="forecasttime">${_converterJs.convertTime(item.dt)}</div>
      <div class="forecastweather">${item.weather[0].main}</div>
      <img class="forecasticon" src="https://openweathermap.org/img/wn/${item.weather[0].icon}.png" alt="">`;
...
function addCity() {
    ...
    location.innerHTML = `<div class = 'addedcitytext'> ${document.querySelector(".cityName").textContent}</div> <div class='crossToDelete'>×</div>`;
    ...
    document.querySelector(".crossToDelete").addEventListener("click", deleteCity);
    let cityName = document.querySelector(".addedcitytext").textContent;
    favoriteCities.push(cityName);
    _storageJs.storage.saveFavoriteCities(favoriteCities);
    console.log(favoriteCities);
}
function renderStoredFavorites(storedFavorits) {
...
function deleteCity() {
    ...
    deleteCityBtn.forEach((item1)=>{
        item1.addEventListener("click", ()=>{
            item1.parentNode.remove();
            ...
            console.log(cities);
            _storageJs.storage.saveFavoriteCities(cities);
        });
    });
}

},{"./apiURL.js":"gUUk7","./storage.js":"kS9sU","./converter.js":"fDRyT","./UI.js":"kSssP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gUUk7":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getUrl", ()=>getUrl
);
function getUrl(cityName, forecast) {
...
function loadStorage() {
    try {
        ...
        _weatherRenderJs.sendWeatherRequest(storedCity);
        _weatherRenderJs.sendForecastRequest(storedCity);
        let storedFavorits = storage.getFavoriteCities();
        storedFavorits.forEach((item)=>{
            _weatherRenderJs.favoriteCities.push(item);
        });
        _weatherRenderJs.renderStoredFavorites(storedFavorits);
    } catch  {
        console.log("No stored Data to output");
    }
}

},{"./weatherRender.js":"22C9S","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fDRyT":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function convertTime(timestamp) {
    const fromUnix = _dateFns.fromUnixTime(timestamp);
    const formattedTime = _dateFns.format(fromUnix, "p");
    return formattedTime;
}
function convertDate(timestamp) {
    const fromUnix = _dateFns.fromUnixTime(timestamp);
    const formattedDate = _dateFns.format(fromUnix, "d LLL");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["e6DQT","bRRRP"], "bRRRP", "parcelRequirea848")
